...
public class CrawlerService {
    ...
    public Integer saveAndStartJob(JobDto jobDto) {
        ...
        Runnable r=new Runnable() {
            ...
            public void run() {
                try{
                    crawler.scrape(jobDto,urlList,filterList,jobStatus.getIdValue());
                }catch (Exception e){
                    e.printStackTrace();
                    scrapeService.markJobFailed(jobStatus.getIdValue());
                }
            }
        };

        jobStatus.setXlsLink(xlsPath+"/"+jobStatus.getIdValue()+"_scrape.xlsx");

        new Thread(r).start();

        return jobStatus.getIdValue();
    }
}
